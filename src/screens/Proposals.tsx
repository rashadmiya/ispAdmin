...
const Proposals = () => {
  let loginUser: UserInterface = useSelector((state: any) => state.loggedInUser.value);
    ...
    useEffect(() => {
        setLoading(true);
        const unsubscribe = firestore().collection('proposals')
    ...
    const updateUserRole = async (notic:any) => {
        ...
        await firestore().collection('notices').doc(notic.id).update({'viewer':viewer})
            .then(() => {
                setLoading(false);
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: ConstantColor.white }}>
            ...
            <View style={global_styles.customContainer}>
                ...
                <View style={{}}>
                    <Text style={[global_styles.textMedium, { fontSize: 18 }]}>Proposals</Text>
                </View>
                <View style={global_styles.sizedBoxTen}></View>
                <View style={global_styles.sizedBoxTen}></View>
                ...
                >
                    {
                       notices.length > 0 ? notices.map((notice: any, index) => (
                            ...
                            <View style={{flex:1, justifyContent:'center', alignItems:'center'}}>
                                <Text style={[global_styles.textMedium, global_styles.textBold,global_styles.textCenter]}>No Proposals Found</Text>
                            </View>
                        )
                    }
